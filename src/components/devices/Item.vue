...
<template>
  <li class="flex items-center justify-between py-6 border-b">
    <div class="flex items-center gap-3">
      <input type="checkbox" id="todo_id_1" class="checkbox" v-model="isChecked" />
      <label for="todo_id_1" class="text-xl cursor-pointer">
    
       <p for="todo_id_1"
          v-if=isChecked class="strikethrough"> {{ device.title }} </p>
      <p for="todo_id_1"
          v-else=isChecked class="text-xl cursor-pointer" > {{ device.title }} </p>
